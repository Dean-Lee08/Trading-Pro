...
// psychology.js 파일에 추가

function updatePsychologyDisplay() {
    const todayData = psychologyData.find(d => d.date === currentTradingDate);
    
    if (!todayData) {
        // 데이터가 없을 때 초기화
        document.getElementById('sleepHoursDisplay').textContent = '0';
        document.getElementById('sleepStatus').textContent = currentLanguage === 'ko' ? '데이터 없음' : 'No data';
        document.getElementById('sleepQualityBar').style.width = '0%';
        
        document.getElementById('environmentScore').textContent = '0';
        document.getElementById('environmentTypeDisplay').textContent = '-';
        document.getElementById('environmentStatus').textContent = currentLanguage === 'ko' ? '데이터 없음' : 'No data';
        document.getElementById('environmentCircle').style.strokeDashoffset = '157';
        
        document.getElementById('stressDisplay').textContent = '3';
        document.getElementById('confidenceDisplay').textContent = '3';
        document.getElementById('focusDisplay').textContent = '3';
        document.getElementById('emotionalStatus').textContent = currentLanguage === 'ko' ? '데이터 없음' : 'No data';
        
        document.getElementById('targetRiskPercent').textContent = '0%';
        document.getElementById('maxLossRiskPercent').textContent = '0%';
        document.getElementById('targetRiskBar').style.width = '0%';
        document.getElementById('maxLossRiskBar').style.width = '0%';
        document.getElementById('riskStatus').textContent = currentLanguage === 'ko' ? '데이터 없음' : 'No data';
    ...
    document.getElementById('sleepHoursDisplay').textContent = sleepHours.toFixed(1);
    
    const sleepQuality = Math.min((sleepHours / 8) * 100, 100);
    document.getElementById('sleepQualityBar').style.width = `${sleepQuality}%`;
    
    let sleepStatus = '';
    if (sleepHours >= 7 && sleepHours <= 9) {
        sleepStatus = currentLanguage === 'ko' ? '최적' : 'Optimal';
    } else if (sleepHours >= 6 && sleepHours < 7) {
        sleepStatus = currentLanguage === 'ko' ? '부족' : 'Insufficient';
    } else if (sleepHours > 9) {
        sleepStatus = currentLanguage === 'ko' ? '과다' : 'Excessive';
    } else {
        sleepStatus = currentLanguage === 'ko' ? '심각하게 부족' : 'Severely Insufficient';
    }
    ...
    const envScore = envScores[todayData.environmentType] || 0;
    document.getElementById('environmentScore').textContent = envScore;
    
    const envTypeLabels = {
        'home': currentLanguage === 'ko' ? '집' : 'Home',
        'office': currentLanguage === 'ko' ? '사무실' : 'Office',
        'cafe': currentLanguage === 'ko' ? '카페' : 'Cafe',
        'hotel': currentLanguage === 'ko' ? '호텔' : 'Hotel'
    };
    document.getElementById('environmentTypeDisplay').textContent = envTypeLabels[todayData.environmentType] || '-';
    document.getElementById('environmentStatus').textContent = envScore >= 80 ? 
        (currentLanguage === 'ko' ? '최적 환경' : 'Optimal Environment') : 
        (currentLanguage === 'ko' ? '개선 필요' : 'Needs Improvement');
    
    const circumference = 2 * Math.PI * 25;
    const offset = circumference - (envScore / 100) * circumference;
    document.getElementById('environmentCircle').style.strokeDashoffset = offset;
    
    // Emotional State
    document.getElementById('stressDisplay').textContent = todayData.stressLevel || 3;
    document.getElementById('confidenceDisplay').textContent = todayData.confidenceLevel || 3;
    document.getElementById('focusDisplay').textContent = todayData.focusLevel || 3;
    
    const avgEmotional = ((6 - todayData.stressLevel) + todayData.confidenceLevel + todayData.focusLevel) / 3;
    let emotionalStatus = '';
    if (avgEmotional >= 4) {
        emotionalStatus = currentLanguage === 'ko' ? '최적 상태' : 'Optimal State';
    } else if (avgEmotional >= 3) {
        emotionalStatus = currentLanguage === 'ko' ? '양호' : 'Good';
    } else {
        emotionalStatus = currentLanguage === 'ko' ? '주의 필요' : 'Needs Attention';
    }
    document.getElementById('emotionalStatus').textContent = emotionalStatus;
    
    // Risk Assessment
    const accountBalance = todayData.accountBalance || 0;
    const dailyTarget = todayData.dailyTarget || 0;
    const maxDailyLoss = todayData.maxDailyLoss || 0;
    
    const targetRiskPct = accountBalance > 0 ? (dailyTarget / accountBalance) * 100 : 0;
    const maxLossRiskPct = accountBalance > 0 ? (maxDailyLoss / accountBalance) * 100 : 0;
    
    document.getElementById('targetRiskPercent').textContent = `${targetRiskPct.toFixed(1)}%`;
    document.getElementById('maxLossRiskPercent').textContent = `${maxLossRiskPct.toFixed(1)}%`;
    
    document.getElementById('targetRiskBar').style.width = `${Math.min(targetRiskPct * 10, 100)}%`;
    document.getElementById('maxLossRiskBar').style.width = `${Math.min(maxLossRiskPct * 10, 100)}%`;
    
    let riskStatus = '';
    if (maxLossRiskPct <= 2 && targetRiskPct <= 3) {
        riskStatus = currentLanguage === 'ko' ? '보수적 (안전)' : 'Conservative (Safe)';
    } else if (maxLossRiskPct <= 5 && targetRiskPct <= 5) {
        riskStatus = currentLanguage === 'ko' ? '적정 리스크' : 'Moderate Risk';
    } else {
        riskStatus = currentLanguage === 'ko' ? '고위험 (주의)' : 'High Risk (Caution)';
    }
