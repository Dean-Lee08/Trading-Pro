[수정본 4.6.html](https://github.com/user-attachments/files/22028033/4.6.html)[Uploading 수<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    <script>
        ...
        function handleTradeSubmit(event) {
            ...
            const shares = formData.amount / formData.buyPrice;
        ...
        function updateBestWorstPerformance(filteredTrades) {
            // Calculate daily P/L
            const dailyStats = {};
        ...
        function updateSymbolPerformance(filteredTrades) {
            ...
            document.getElementById('symbolPerformanceList').innerHTML = symbolPerfHtml || `
                <div class="empty-state" style="padding: 20px;">
                    <div style="color: #64748b; font-size: 13px;" data-lang="no-symbol-data">${translations[currentLanguage]['no-symbol-data'] || 'No symbol data available'}</div>
                </div>
            `;
        ...
        function showDayDetails(dateString, dayTrades) {
            ...
            if (dayTrades.length === 0) {
                ...
                dayTrades.sort((a, b) => {
                    const timeA = a.exitTime || '23:59';
                    const timeB = b.exitTime || '23:59';
                    return timeB.localeCompare(timeA);
                });
                
                dayTradesList.innerHTML = dayTrades.map(trade => `
                    <div class="day-trade-item">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                            <span class="trade-symbol">${trade.symbol}</span>
                            <span class="trade-pnl ${trade.pnl >= 0 ? 'positive' : 'negative'}">${trade.pnl.toFixed(2)}</span>
                        </div>
                        <div style="font-size: 11px; color: #64748b;">
                            ${trade.buyPrice.toFixed(4)} → ${trade.sellPrice.toFixed(4)} | ${trade.holdingTime || 'N/A'}
                        </div>
                    </div>
                `).join('');
            }
        ...
        function updateWeeklySummary() {
            ...
            summaryContainer.innerHTML = weeks.map((week, index) => {
                const weekTrades = trades.filter(trade => {
                    const tradeDate = new Date(trade.date);
                    return tradeDate >= week.start && tradeDate <= week.end;
                });
                
                const totalPL = weekTrades.reduce((sum, trade) => sum + trade.pnl, 0);
                const tradeCount = weekTrades.length;
                
                const weekLabel = currentLanguage === 'ko' ? `${index + 1}주차` : `Week ${index + 1}`;
                
                return `
                    <div class="weekly-item">
                        <span class="weekly-label">${weekLabel}</span>
                        <span class="weekly-value ${totalPL >= 0 ? 'positive' : 'negative'}">
                            ${totalPL.toFixed(2)} (${tradeCount})
                        </span>
                    </div>
                `;
            }).join('');
